...
def _prepare_final_comparison_prompt(
    ...
    # Определяем рыночные данные для города
    market_info = ""
    city_lower = city.lower()
    if "минск" in city_lower:
        market_info = "РЫНОК 2025 Минск: средняя цена $1200-2500/м². Высокий спрос, премиум сегмент."
    elif any(x in city_lower for x in ["брест", "гомель", "витебск", "гродно", "могилев"]):
        market_info = "РЫНОК 2025 Областные центры: средняя цена $600-1200/м². Стабильный рынок."
    else:
        market_info = "РЫНОК 2025 Районные города: средняя цена $400-800/м². Более доступный сегмент."
...
📊 ТЕКУЩАЯ СИТУАЦИЯ НА РЫНКЕ {city.upper()}:
{market_info}

ВАЖНО: Сравнивай лучшие варианты НЕ ТОЛЬКО между собой, но и с текущей ситуацией на рынке {city}:
• Какие цены за м² сейчас на рынке {city}?
• Насколько эти варианты выгодны по сравнению с типичными предложениями в {city}?
• Есть ли среди них действительно выгодные предложения или все в пределах нормы?
• Какие факторы делают вариант особенно привлекательным на фоне рынка {city}?

⚠️ КРИТИЧЕСКИ ВАЖНО — ПРОВЕРКА НА ДОЛЮ/КОМНАТУ/ЧАСТЬ ДОМА:
...
- "часть дома", "часть дома имеет статус квартиры", "квартира в частном доме", "часть частного дома", "квартира в доме" (если это часть частного дома, а не квартира в многоквартирном доме)

НЕ считай долей/комнатой/частью дома если:
...
- В названии указано "квартира", "студия", "апартаменты" И это квартира в многоквартирном доме

Если ТОЧНО найдено → укажи в critical_notes: "⚠️ ДОЛЯ: [детали]" или "⚠️ КОМНАТА: [детали]" или "⚠️ ЧАСТЬ ДОМА: [детали]"
Если доля/комната/часть дома → снизь final_score на 3-4 балла, НЕ выбирай в топ если есть целые квартиры в многоквартирных домах!

ОЦЕНКА (1-10) - сравнивай с ТЕКУЩИМ РЫНКОМ {city.upper()}:
ВАЖНО: Оценивай каждый вариант относительно текущей рыночной ситуации в {city}, а не только между собой!

• Цена vs текущий рынок {city}:
  - 10 баллов: цена ЗНАЧИТЕЛЬНО ниже рынка (на 15-20% и более) - отличная сделка
  - 8-9 баллов: цена ниже рынка (на 5-15%) - хорошая сделка
  - 6-7 баллов: цена в пределах рынка (соответствует средним ценам) - нормальная сделка
  - 4-5 баллов: цена выше рынка (на 5-15%) - завышена
  - 1-3 балла: цена ЗНАЧИТЕЛЬНО выше рынка (на 15% и более) - сильно завышена

• Год постройки (учитывай на фоне рынка {city}):
...
• ЦЕЛАЯ КВАРТИРА (доля/комната/часть дома = -3-4 балла):
  - Только целые квартиры в многоквартирных домах получают полную оценку
  - Доли/комнаты/части домов должны быть исключены из топ-результатов, если есть альтернативы

• Соотношение цена/качество на фоне рынка {city}:
  - Учитывай: состояние ремонта, площадь, этаж, наличие балкона/лифта и т.д.
  - Сравнивай с типичными предложениями в {city} за аналогичную цену

• Уникальность предложения:
  - Есть ли что-то особенное, что выделяет вариант на рынке {city}?
  - Редкое сочетание параметров (например, низкая цена + хорошее состояние + хороший район)?

Выбери ТОП-{max_results} ЛУЧШИХ из этих вариантов (ТОЛЬКО ЦЕЛЫЕ КВАРТИРЫ!), учитывая их преимущества НА ФОНЕ ТЕКУЩЕГО РЫНКА {city.upper()}!
...
{{
  "analysis_summary": "Краткая сводка: сравнение {len(best_from_batches)} лучших вариантов из батчей с учетом текущей рыночной ситуации в {city}, какие варианты особенно выгодны на фоне рынка {city}, общая оценка рынка {city}",
  "top_offers": [
    {{
      "offer_id": "kufar_123456",
      "title": "Краткое описание",
      "final_score": 9.5,
      "reason": "Детальное обоснование выбора с учетом сравнения с текущим рынком {city}: почему этот вариант выгоден/не выгоден на фоне рынка {city}, как цена соотносится с типичными предложениями в {city}, что делает его особенным (2-3 предложения)",
      "critical_notes": "Важные замечания (если есть доля/комната/часть дома - укажи здесь). Также укажи, если цена завышена относительно рынка {city} или есть другие важные недостатки."
    }}
  ]
}}
...
{{
  ...
  "top_offers": [
    {{
      ...
      "critical_notes": ["⚠️ ОБЯЗАТЕЛЬНО укажи здесь ТОЛЬКО если это ТОЧНО ДОЛЯ, КОМНАТА или ЧАСТЬ ДОМА (не целая квартира в многоквартирном доме)! Формат: '⚠️ ДОЛЯ: 1/2 в двухкомнатной квартире' или '⚠️ КОМНАТА: комната в коммуналке' или '⚠️ ЧАСТЬ ДОМА: часть частного дома'. Если это ЦЕЛАЯ КВАРТИРА В МНОГОКВАРТИРНОМ ДОМЕ — НЕ указывай долю/комнату/часть дома, укажи другие недостатки или 'Нет критических недостатков'", "Другие недостатки (если есть)"]
    }}
  ],
  "best_overall": {{
