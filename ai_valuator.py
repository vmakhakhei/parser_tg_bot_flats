...
class AIValuator:
    ...
    def _prepare_prompt(self, listing: Listing, inspection: Optional[Dict[str, Any]] = None) -> str:
        ...
        prompt = f"""Оценщик недвижимости Беларуси. Оцени квартиру:
...
РЫНОК 2025 ($/м²): Центр $650-840, Боровки $510-620, Северный $480-580, Южный $450-550, Восточный $470-590, Текстильный $440-510
Корректировки: 1й/последний этаж -5%, до 1980г -7%, евроремонт +10%, требует ремонта -10%

ЗАДАЧА: Определи справедливую цену, сравни с рынком, дай рекомендации.
...
def _prepare_selection_prompt_detailed(
    ...
    prompt = f"""Эксперт по недвижимости. Выбери {max_results} лучших квартир по цена/качество.

ФИЛЬТРЫ: {min_rooms}-{max_rooms} комнат, ${min_price:,}-${max_price:,}

РЫНОК ($/м²): Центр $650-840, Боровки $510-620, Северный $480-580, Южный $450-550, Восточный $470-590, Текстильный $440-510

КРИТЕРИИ: 1) Цена за м² vs рынок района 2) Год постройки (после 2010 +бонус) 3) Состояние ремонта 4) Район

ОБЪЯВЛЕНИЯ:
{''.join(listings_text)}

ЗАДАЧА: Выбери минимум 5 лучших вариантов. Для каждого напиши объяснение (3-5 предложений): цена за м², сравнение с рынком района, год постройки, состояние, почему выгодно.
