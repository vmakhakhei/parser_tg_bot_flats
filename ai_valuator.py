...
class AIValuator:
    ...
    def _prepare_prompt(self, listing: Listing) -> str:
        ...
        prompt = f"""Ты профессиональный оценщик недвижимости с опытом работы на рынке Барановичей, Беларусь.
...
5. Состояние: если не указано, считаем средним

ЗАДАЧА:
1. Рассчитай справедливую рыночную стоимость квартиры в USD, учитывая все факторы
2. Определи, завышена ли цена: true если текущая цена > справедливая цена + 10%, иначе false
3. Дай краткую оценку на русском языке (1-2 предложения), объясни почему цена справедлива или завышена

ВАЖНО: Ответь ТОЛЬКО валидным JSON без дополнительного текста до или после:
{{
    "fair_price_usd": число,
    "is_overpriced": true/false,
    "assessment": "текст оценки на русском языке"
}}"""
        return prompt
...
Твоя задача - давать точные, объективные оценки стоимости квартир на основе рыночных данных.
Всегда отвечай ТОЛЬКО валидным JSON без дополнительного текста.
Формат ответа: {"fair_price_usd": число, "is_overpriced": true/false, "assessment": "текст на русском"}"""
                },
