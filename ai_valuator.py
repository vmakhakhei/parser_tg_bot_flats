...
async def select_best_listings(
    ...
    try:
        # Инспектируем параллельно (но ограничиваем количество для скорости)
        inspect_limit = min(50, len(listings_to_inspect))  # Максимум 50 объявлений для инспекции
        inspection_tasks = []
...
Перед оценкой ОБЯЗАТЕЛЬНО проверь название и описание каждого объявления на наличие:
• ДОЛЯ: "доля", "1/2", "1/3", "1/4", "часть квартиры", "доля собственности", "половина квартиры"
• КОМНАТА: "комната в", "комната в коммуналке", "комната в общежитии", "комната"
• НЕ ЦЕЛАЯ: "не целая квартира", "часть квартиры"

ПРАВИЛА:
- Если найдено упоминание доли/комнаты → ОБЯЗАТЕЛЬНО укажи в critical_notes: "⚠️ ДОЛЯ: [детали]" или "⚠️ КОМНАТА: [детали]"
- Если это доля/комната → СНИЗЬ final_score на 3-4 балла (например, с 8.5 до 4.5-5.5)
- Если это доля/комната → НЕ выбирай в топ, если есть целые квартиры с похожей ценой
- Пользователь ищет ЦЕЛУЮ КВАРТИРУ, а не долю/комнату!
...
1. Оцени каждое объявление по критериям (1-10):
   ...
   • ⚠️ ЦЕЛАЯ КВАРТИРА: если это доля/комната — снизь оценку на 3-4 балла!

2. Сравни все объявления между собой

3. Выбери ТОП-{max_results} лучших вариантов (ТОЛЬКО ЦЕЛЫЕ КВАРТИРЫ! Доли/комнаты исключить, если есть альтернативы)
