...
def _prepare_selection_prompt_detailed(
    ...
    prompt = f"""Ты - эксперт по недвижимости Барановичей с 10+ летним опытом. Твоя задача - выбрать МИНИМУМ 5, максимум {max_results} ЛУЧШИХ вариантов квартир по критерию "цена-качество" для покупателя.
...
3. Выбери МИНИМУМ 5 вариантов с НАИБОЛЬШИМ общим баллом

4. Для каждого выбранного варианта напиши МАКСИМАЛЬНО ДЕТАЛЬНОЕ И ПОДРОБНОЕ объяснение (8-12 предложений) с конкретными цифрами, фактами и сравнениями:

   ОБЯЗАТЕЛЬНО включи все 6 разделов:

   1. ЦЕНА И СООТНОШЕНИЕ ЦЕНА/КАЧЕСТВО (2-3 предложения):
      - Конкретная цена за м² (например: "$520/м²")
      - Сравнение со средним по району в процентах и долларах (например: "на 15% ниже среднего по району Боровки ($600/м²), экономия $80/м²")
      - Сравнение с другими вариантами из списка (лучше/хуже и на сколько)
      - Почему это хорошее соотношение цена/качество

   2. ГОД ПОСТРОЙКИ И ТЕХНИЧЕСКОЕ СОСТОЯНИЕ (2-3 предложения):
      - Конкретный год постройки
      - Влияние года на стоимость (например: "дом 2014 года - современные технологии, энергоэффективность, что добавляет +5-10% к стоимости")
      - Техническое состояние (электропроводка, сантехника, отопление)
      - Потенциальные затраты на ремонт/замену коммуникаций

   3. РАЙОН И РАСПОЛОЖЕНИЕ (2-3 предложения):
      - Конкретный район и его характеристики
      - Преимущества района (инфраструктура, транспорт, школы, магазины)
      - Недостатки района (если есть)
      - Сравнение с другими районами из списка (лучше/хуже и почему)

   4. СОСТОЯНИЕ РЕМОНТА И ВНУТРЕННЯЯ ОТДЕЛКА (1-2 предложения):
      - Детальное описание состояния ремонта (из описания и фото)
      - Качество отделки (стены, пол, потолок, сантехника)
      - Нужен ли дополнительный ремонт и сколько это может стоить

   5. ОСОБЕННОСТИ И ПРЕИМУЩЕСТВА (1-2 предложения):
      - Конкретные особенности (балкон, лоджия, парковка, лифт, охрана)
      - Планировка (изолированные комнаты, размер кухни, санузлы)
      - Дополнительные преимущества (вид из окон, этаж, подъезд)

   6. СРАВНЕНИЕ С ДРУГИМИ ВАРИАНТАМИ И ОБЩАЯ ОЦЕНКА (2-3 предложения):
      - Чем этот вариант ЛУЧШЕ других из списка (конкретные преимущества)
      - Почему он выделяется среди остальных
      - Для кого этот вариант подходит лучше всего
      - Общая рекомендация и вывод

ВАЖНО:
- Выбери МИНИМУМ 5 вариантов (если есть столько подходящих), максимум {max_results}
- Каждое объяснение должно быть МАКСИМАЛЬНО ДЕТАЛЬНЫМ (8-12 предложений) с конкретными цифрами, фактами и сравнениями
- Не пиши общие фразы - пиши КОНКРЕТНО с цифрами и сравнениями
- ОБЯЗАТЕЛЬНО включи все 6 разделов выше для каждого варианта
- Сравнивай каждый вариант с другими из списка, объясняй почему он лучше

ПРИМЕР ПРАВИЛЬНОГО ДЕТАЛЬНОГО ОБЪЯСНЕНИЯ (минимум 8-12 предложений):
"Этот вариант является отличным выбором благодаря своей привлекательной цене в $520/м², что на 15% ниже средней цены для района Боровки ($600-620/м²), что означает экономию в $80-100 за каждый квадратный метр. При общей площади 50 м² это составляет экономию в $4000-5000 по сравнению со среднерыночной стоимостью в этом районе. Квартира расположена в современном районе Боровки, который известен своими новыми панельными домами, построенными после 2010 года. Год постройки 2014 обеспечивает современные технологии строительства, энергоэффективные материалы и новые коммуникации, что добавляет 5-10% к стоимости недвижимости и снижает будущие затраты на ремонт. Район Боровки отличается развитой инфраструктурой: рядом находятся новые школы, детские сады, магазины и остановки общественного транспорта, что делает его привлекательным для семей с детьми. Состояние ремонта оценивается как хорошее: по фотографиям видно, что квартира имеет современную отделку, новую сантехнику и качественные материалы, что позволит новому владельцу сэкономить на первоначальных вложениях в ремонт. Особенности квартиры включают наличие балкона, что добавляет функциональности и увеличивает полезную площадь, а также наличие лифта в доме, что особенно важно для пожилых людей или семей с колясками. По сравнению с другими вариантами из списка, этот выделяется оптимальным сочетанием современного года постройки, хорошего района и привлекательной цены за м², что делает его лучшим выбором для тех, кто ищет квартиру с хорошим соотношением цена-качество в современном районе."

ПРИМЕР НЕПРАВИЛЬНОГО КОРОТКОГО ОБЪЯСНЕНИЯ (НЕ ДЕЛАЙ ТАК):
"Хорошая цена за м²: $557/м². Соответствует критериям поиска."
"Хорошее соотношение цена-качество."

КРИТИЧЕСКИ ВАЖНО:
- Каждое объяснение ДОЛЖНО быть минимум 8-12 предложений (150-300 слов)
- ОБЯЗАТЕЛЬНО включи все 6 разделов выше для каждого варианта
- Используй КОНКРЕТНЫЕ ЦИФРЫ и сравнения (не общие фразы)
- Сравнивай каждый вариант с другими из списка
- Объясняй, почему именно этот вариант лучше остальных

Верни ОТВЕТ В ФОРМАТЕ JSON:
{{"selected": [{{"id": "listing_id", "reason": "МАКСИМАЛЬНО ДЕТАЛЬНОЕ И ПОДРОБНОЕ объяснение МИНИМУМ 8-12 предложений (150-300 слов) с конкретными цифрами, фактами и сравнениями. ОБЯЗАТЕЛЬНО включи все 6 разделов: 1) цена и соотношение цена/качество с сравнениями в процентах и долларах, 2) год постройки и техническое состояние с оценкой влияния на стоимость, 3) район и расположение с конкретными преимуществами и сравнением с другими районами, 4) детальное состояние ремонта из описания и фото, 5) конкретные особенности и преимущества квартиры, 6) сравнение с другими вариантами из списка и объяснение почему этот вариант лучше. Пиши КОНКРЕТНО с цифрами, не общими фразами. Смотри пример правильного объяснения выше."}}]}}

Только JSON, без дополнительного текста. ОБЯЗАТЕЛЬНО минимум 5 вариантов. КАЖДОЕ объяснение должно быть минимум 8-12 предложений."""
