...
def _prepare_selection_prompt_detailed(
    ...
    for i, item in enumerate(listings_to_process, 1):
        ...
        # Сокращаем до 150 символов для уменьшения размера промпта
        inspection = item.get("inspection", {})
        full_description = inspection.get("full_description", "") if inspection else ""
        description_text = full_description[:150] if full_description else (listing.description[:150] if listing.description else "")
...
⚠️ ВАЖНО — ПРОВЕРКА НА ДОЛЮ/КОМНАТУ:
Проверь название и описание на: "доля", "1/2", "1/3", "1/4", "комната в", "часть квартиры"
Если найдено → укажи в critical_notes: "⚠️ ДОЛЯ: [детали]" или "⚠️ КОМНАТА: [детали]"
